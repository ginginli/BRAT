...
document.addEventListener('DOMContentLoaded', function() {
    ...
    function initializeGenerator() {
        ...
        // 确保有一个有效的meme-image (懒加载处理)
        let memeImage = document.getElementById('meme-image');
        if (memeImage && memeImage.getAttribute('data-src')) {
            // 主动加载生成器图片而不等待懒加载
            const src = memeImage.getAttribute('data-src');
            if (src) {
                ...
                memeImage.src = src;
                memeImage.removeAttribute('data-src');
            } else {
                initializeApp();
            }
        } else {
